...
// 3, record the prime numbers and only check prime factors
...
int main() {
  ...
  int i, j;
  bool prime;
  for (i = 2; i <= 100; i++) { // begin from 2
    prime = true;
    for (j = 2; j < i; j++) {  // also begin from 2
      if (i % j == 0) {
        prime = false; 
      }
    }
  ...
  std::vector<int>  primeVec={2}; // record primes in a vector
  for (i = 3; i <= 100 ; i=i+2) { // begin from 3 and only check odd numbers
    ...
    if (prime) {
      primeVec.push_back(i);  // record the prime numbers
      std::cout << i << " ";
    }
  }
  std::cout << std::endl;
